...
def fetch_company_link():
    ...
    fields[
        "__EVENTTARGET"
    ] = "ctl00:contentPlaceHolderConteudo:BuscaNomeEmpresa1:btnTodas"
...
def fetch_dfp_urls(url):
    ...
    urls = re.search(",(?P<data>\[(.*)\])", data.find_all("script")[-2].text).group(
        "data"
    )
...
def _create_date(real_date):
    from bmf.models import Date

    date_obj, created = Date.objects.get_or_create(
        day=real_date.day, month=real_date.month, year=real_date.year, date=real_date
    )
    if created:
...
def _fetch_report_page():
    ...
    from bmf.models import FinancialReport, REPORT
    ...
    for member in Company.objects.filter(ibovespa=False):
        ...
        for date, payload in links.items():
            ...
            for kind, report_url in fetch_dfp_urls(main_url).items():
                print(
                    "Financial Report - {} from {} on {}".format(
                ...
                for key, value in REPORT:
                    if kind == value:
                        ...
                        if created:
                            print(
                                "Creating report {} for {}".format(
                                    date_obj.date, member.name
                                )
                            )
                        break
                print("-" * 100)
